...
const PendingRiders = () => {
  ...
  const handleApprove = async (id, email) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await axiosSecure.patch(`/riders/update-status/${id}`, { action: "approve", email });
        refetch()
  ...
  const handleDecline = async (id, email) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        await axiosSecure.patch(`/riders/update-status/${id}`, { action: "decline", email });
        ...
        <dialog open className="modal">
          <div className="modal-box max-w-xl relative">
            ...
            <div className="modal-action justify-start gap-2">
              ...
              <button
                onClick={() => handleDecline(selectedRider._id, selectedRider.email)}
